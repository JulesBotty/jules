...
const createUserByAdmin = async (req, res) => {
  ...
  const user = await UserModel.create({ name, email, password, role, poste });
  res.status(201).json({
...
const getAnalytics = async (_req, res) => {
  const [admins, managers, employees, totalLeaves, pendingLeaves, approvedLeaves, rejectedLeaves, Maritimes, Aeriens, Terrestres, Prestations] =
    await Promise.all([
  ...
  res.json({
    totals: { totalUsers, admins, managers, employees, Maritimes, Aeriens, Terrestres, Prestations, totalDossier },
    leaves: {
      total: totalLeaves,
